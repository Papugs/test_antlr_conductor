...
// Mark sweep vars
const MARKBYTE = 7; // use unused bit
const MARKED = 128; // highest bit is 1, 2^7
...
const mark = (address) => {
    // Check marked
    const tag_byte = heap_get_byte_at_offset(address, MARKBYTE);
    if (tag_byte & MARKED) {
        return;
    }
    
    // Mark
    heap_set_byte_at_offset(address, MARKBYTE, tag_byte | MARKED);
    
    // Mark children recursively
    const num_children = heap_get_number_of_children(address);
    for (let i = 0; i < num_children; i++) {
        mark(heap_get_child(address, i));
    }
}
...
Test case: ` + program + "\n")
    const result = 
        parse_compile_run(program, heapsize)
...
test(`
function create_many_objects(n) {
    if (n <= 0) {
        return 0;
    } else {
        return 1 + create_many_objects(n - 1);
    }
}
create_many_objects(60); // Large enough to exhaust initial free list
// can be demonstrated by uncommenting console.log in sweep()
`, 60, 5000);
